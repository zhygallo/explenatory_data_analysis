...
def main():
    ...
    if df is not None:
        ...
        st.sidebar.subheader("Optional. Data Profiling")
        st.sidebar.write(
            "This step will generate basic statistics about your data and will help you understand it better"
        )
        st.sidebar.write("It is optional, so feel free to skip it")
        show_profiling = st.sidebar.radio(
            label="Profile?", options=["Yes", "No"], index=1
        )
        ...
        if st.sidebar.button("Classify", key="classify") and (
            ...
            st.write(
                "Precision: ",
                precision_score(y_test, y_pred, average="micro").round(2),
            )
